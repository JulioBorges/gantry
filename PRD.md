...
Parse the scope (`<spec-slug> | spec#NN | wave:N | frontier | all | "goal"`), `--limit` (issues per round, default 4) and `--budget` (correction budget, default from policy, 2). Refuse a dirty tree. Offer a dedicated worktree on branch `<prefix>/<scope-slug>` from the target branch — always, before anything is created. Ask once which model runs Plan, Implement, Review and Critic (Critic and Plan at least as strong as Implement). Run `roadmap.py check`; drift is fixed with the operator before anything else. Open the run in the run log with the policy hash, the harness and its support tier.
