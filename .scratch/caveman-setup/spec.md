...
- When installation is needed, show the upstream skill-only installation command appropriate to the user's harness. The user runs it themselves. After the user reports installation, verify that the host harness can discover and read the Caveman skill. A successful installation claim or package-manager exit code alone does not prove availability.
